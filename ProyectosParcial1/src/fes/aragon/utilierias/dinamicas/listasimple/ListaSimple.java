...
public class ListaSimple<E> {
	...
	public int estaEnLista(E dato) {
		...
		Nodo<E> tmp=null;
		tmp=cabeza;
		for(indice = 0; indice < longitud-1 && tmp != null && tmp.getDato().equals(dato); indice++, tmp = tmp.getSiguiente()) 
		;
			if (tmp != null) {
				return indice;
			}else {
	...
	public void eliminarNodo(int indice) {
		...
		if (indice<=longitud) {
			for(int contador=0; contador < indice && tmp !=null; contador++,pred=tmp, tmp=tmp.getSiguiente());	
			
		}
		System.out.println("act= "+tmp.getDato());
		System.out.println("Ant= "+pred.getDato());
		
		if(tmp!=null) {
			if(indice==0) {
				cabeza=cabeza.getSiguiente();
			}else 
				pred.setSiguiente(tmp.getSiguiente());
			}
	...
	public boolean eliminar(E dato) {
		...
		if(cabeza != null) {
			if (cabeza == cola && dato.equals(cabeza.getDato())) {
				...
				Nodo<E> prd, tmp;
				for(prd=cabeza, tmp=cabeza.getSiguiente(); tmp !=null && !tmp.getDato().equals(dato);
	...
	public boolean eliminarEnIndice(int indice) {
		...
		if(indice>=0 && indice <=longitud-1) {
			...
			if(cabeza==cola && indice ==0) {
				...
				Nodo<E> prd, tmp;
				int contador=1;
				for(prd=cabeza, tmp=cabeza.getSiguiente(); contador<indice; prd= prd.getSiguiente(),
	...
	public void asignarVarios(E dato, E nuevaDato, boolean todos) {
		Nodo<E> tmp =null;
		if(!todos) {
			for(tmp=cabeza; tmp != null; tmp=tmp.getSiguiente()) {
				if(tmp.getDato().equals(dato)) {
					tmp.setDato(nuevaDato);
					return;
				}
			}
		}else {
	...
	public E eliminar_ObtenerEnCabeza() {
		Nodo<E> tmp=null;
		if(cabeza!=null) {
			if(cabeza==cola) {
				tmp=cabeza;
